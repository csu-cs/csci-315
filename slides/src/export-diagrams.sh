...
for texfile in "${OUTDIR}"/*.tex; do
  ...
  overlays=$(grep -oP '\\(only|onslide|alt)<\K[0-9,\-]+' "$texfile" | tr ',' '\n' | \
    perl -ne '
      while (/(\d+)(?:-(\d+))?/g) {
        if (defined $2) {
          print "$_\n" for $1..$2;
  ...
  if [[ -z "$overlays" ]]; then
    ...
    [[ -f "${OUTDIR}/${base}.svg" ]] && rm -f "$texfile" "${OUTDIR}/${base}.log" "${OUTDIR}/${base}.aux"
  else
...
# Change the height and width of all SVGs to be 150% of their original size using sed.
for svg in "$OUTDIR"/*.svg; do
  if [ -f "$svg" ]; then
    ...
    if [[ $width =~ ^[0-9.]+$ && $height =~ ^[0-9.]+$ ]]; then
      new_width=$(awk "BEGIN {printf \"%.3f\", $width * 1.5}")
      new_height=$(awk "BEGIN {printf \"%.3f\", $height * 1.5}")
