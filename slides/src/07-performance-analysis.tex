...
\begin{frame}{Approaches to Analyzing Performance}
  ...
  \begin{enumerate}
  \item<2->
    \textbf{Empirical}: Measure performance for several examples.\\But, what does this tell us in general?
  \item<3->
    \textbf{Analytical}:
    \begin{itemize}
    \item<4-> Instead, assess performance in an abstract, generalized manner.
...
\begin{frame}{Analytical Approach: Step 1 of 2}

Characterize performance by counting the number of of key operations.

\begin{itemize}
\item <2->
  For a \emph{sorting} algorithm, the key operations includes\\the number of times:
  \begin{itemize}
  \item
...
\begin{itemize}
\item Usually, we approximate or estimate the performance of an algorithm, assuming it is operating on a very large data set.
\item Estimation is an important skill to learn and use.
\end{itemize}
\end{frame}
...
%\begin{block}{}
\centerline{\includegraphics[width=0.7\textwidth]{performance-growth-rate1}}
%\end{block}
...
\toprule{}
  \uncover<3->{$n$}  & $50n^2 + 31n^3 + 24n + 15$ & $3n^2 + n + 21 + 4 \cdot 3^n$ \\
\midrule{}
  \onslide<3->{  1   &        120     &            37 }\\
  \onslide<3->{  2   &        511     &            71 \\}
...
\onslide<2->{
  \begin{tabular}{r @{\hskip 0.75cm} r r c }
    \toprule{}
    $n$  & $3n^2 + n + 21 + 4 \cdot 3^n$  & $4 \cdot 3^n$ & Proportion \\
    \midrule{}
    1  &          37  &         12  &      32.4\% \\
    2  &          71  &         36  &      50.7\% \\
    3  &         159  &        108  &      67.9\% \\
...
\begin{frame}{Growth rates of common complexity functions}
  \centering\includegraphics[width=\textwidth]{big-o-growth-rates}
\end{frame}
...
\begin{frame}[fragile]{Example 4: Recursion can be hard.}
  ...
  \centering\includegraphics[width=0.7\textwidth]{towers-of-hanoi}
\end{frame}
...
\item When $n>0$,
      $T(n) = c_1 + c_2 + T(n-1) + c_3 + c_4 + T(n-1)$ \\
           ...
           = $2 \cdot T(n-1) + c$   $\to$ recurrence equation for the growth-rate function of hanoi-towers algorithm. \\
...
\begin{lstlisting}[basicstyle=\scriptsize\ttfamily\bfseries,identifierstyle=\color{white}]
1000  4e-06
2000  8e-06
3000  1.2e-05
4000  2.5e-05
...
\begin{frame}{Opening PDF files in Visual Studio Code}
\begin{itemize}
  \item
...
\end{itemize}
\column{0.4\textwidth}
\centerline{\Colorbox{white}{\includegraphics[width=1.0\textwidth]{gnuplot-timings2}}}
\end{columns}
\end{frame}
