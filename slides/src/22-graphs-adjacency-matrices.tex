...
\subtitle{CSCI 315: \textit{Data Structure Analysis}\\Chapter 20}
...
\section*{Intro}
\begin{frame}{The Seven Bridges of K{\"o}nigsberg}
The Pregel River flows around the island of Kneiphof\\
...
Over the past 290 years, graph theory has been applied \\
to a variety of problems, including:
\begin{itemize}
  \item  large language models (AI)
  \item  linguistics,
...
\begin{itemize}
  \item
    \emph{Member} (\(x \in A\)): \(x\) is an element of the set \(A\).
  \item
    \emph{Subset} (\(A \subseteq B\)): every element of \(A\) that is an\\element of \(B\).
  \item
    \emph{Intersection} (\(A \cap B\)): all the elements in \(A\) and \(B\).\\
    \(A \cap B = \{x \mid x \in A\) and \(x \in B\} \)
  \item
    \emph{Union} (\(A \cup B\)): all the elements that are in \(A\) or in \(B\).\\
    \(A \cup B = \{x \mid x \in A\) or \(x \in B\} \)
  \item
    \emph{Ordered Pairs} (\(A \times B\)): all the ordered pairs of elements of \(A\) and \(B\).\\
...
\begin{figure}
\centering
\begin{tikzpicture}[very thick, -stealth, shorten >=0.5pt]
  \node[state] (A)                      {A};
...
\begin{tikzpicture}[very thick,font=\bfseries\tiny, node distance=2cm, every state/.style={minimum size=1.25cm, inner sep=2pt, fill=CSUcyan, text=black, very thick}]
    \node[state] (San Francisco)           {San\\Francisco};
    \node[state] (Denver) [right of=San Francisco, node distance=4cm] {Denver};
    ...
    \path (Charleston) edge (Baltimore)
                       ...
                       edge (Nashville)
                       edge (Denver)
                       edge [bend left=10] (San Francisco)
          (Nashville)  edge (Chicago)
                       edge (Baltimore)
                       edge [bend right](San Francisco)
          (Chicago)    edge (New York)
                       edge (Baltimore)
          (Dallas)     edge (San Francisco)
                       edge (Denver)
                       edge (Orlando)
          (Denver)     edge (San Francisco)
                       edge (Chicago);
\end{tikzpicture}
\end{figure}
\end{frame}
...
\begin{itemize}
  \item
    ...
    \end{itemize}
  \item
    \emph{Adjacent}: there is an edge from one vertex to the other; i.e., \((v, w) \in E(G)\)
  \item
    \emph{Loop}: edge to and from a single vertex, \(e = (v, v)\)
  \item
...
\begin{itemize}
  \item
    \emph{Connected vertices}: there is a path from \(w\) to \(v\).
  \item
    \emph{Path} from \(w\) to \(v\) is if there is sequence of vertices \(w_1, w_2, \ldots, w_n\) such that \(w = w_1\), \(w_n = v\), and \((w_i, w_i + 1)\) is an edge for all \(i = 1, 2, \ldots, n - 1\).
  \item
...
\begin{itemize}
  ...
  \item
    If there is an edge from \(w\) to \(v\) (i.e., \((w, v) \in E(G)\)), then \(w\) is \emph{adjacent to} \(v\) and \(v\) is \emph{adjacent from} \(w\).
  \item
...
\begin{frame}[b]{Adjacency Matrix: Undirected Example}
%\centering
\begin{columns}
  ...
  \column{0.55\textwidth}
    \begin{tabular}{r | B | B | B | B | B |}
      ...
      \arraySubscript{0}
        & \mFal{} & \mTru{} & \mTru{} & \mFal{} & \mFal{} \\\hhline{~|-----|}
      \arraySubscript{1}
        & \mTru{} & \mFal{} & \mTru{} & \mTru{} & \mTru{} \\\hhline{~|-----|}
      \arraySubscript{2}
        & \mTru{} & \mTru{} & \mFal{} & \mTru{} & \mTru{} \\\hhline{~|-----|}
      \arraySubscript{3}
        & \mFal{} & \mTru{} & \mTru{} & \mTru{} & \mFal{} \\\hhline{~|-----|}
      \arraySubscript{4}
        & \mFal{} & \mTru{} & \mTru{} & \mFal{} & \mFal{} \\\hhline{~|-----|}
    \end{tabular}
...
\begin{frame}[b]{Adjacency Matrix: Directed Example}
% \centering
\begin{columns}
  \column{0.33\textwidth}
    %\begin{figure}
  ...
  \column{0.55\textwidth}
    \begin{tabular}{r | B | B | B | B | B |}
      ...
      \arraySubscript{0}
        & \mFal{} & \mTru{} & \mTru & \mFal & \mFal \\\hhline{~|-----|}
      \arraySubscript{1}
        & \mFal{} & \mFal{} & \mTru & \mTru & \mTru \\\hhline{~|-----|}
      \arraySubscript{2}
        & \mTru{} & \mFal{} & \mFal & \mTru & \mTru \\\hhline{~|-----|}
      \arraySubscript{3}
        & \mFal{} & \mFal{} & \mFal & \mTru & \mFal \\\hhline{~|-----|}
      \arraySubscript{4}
        & \mFal{} & \mFal{} & \mFal & \mFal & \mFal \\\hhline{~|-----|}
    \end{tabular}
...
\begin{frame}[b]{Adjacency Matrix: Weighted Directed Example}
% \centering
\begin{columns}
  \column{0.33\textwidth}
    %\begin{figure}
  ...
  \column{0.55\textwidth}
    \begin{tabular}{r |  B | B | B | B | B |}
      \multicolumn{1}{c}{}
        ...
        & \multicolumn{1}{c}{\arraySubscript{4}}\\\hhline{~|-----|}
      \arraySubscript{0}
        & \mFal[\textInf] & \mTru[10] & \mTru[1] & \mFal[\textInf] & \mFal[\textInf] \\\hhline{~|-----|}
      \arraySubscript{1}
        & \mFal[\textInf] & \mFal[\textInf] & \mTru[2] & \mTru[4] & \mTru[1] \\\hhline{~|-----|}
      \arraySubscript{2}
        & \mTru[3] & \mFal[\textInf] & \mFal[\textInf] & \mTru[1] & \mTru[5] \\\hhline{~|-----|}
      \arraySubscript{3}
        & \mFal[\textInf] & \mFal[\textInf] & \mFal[\textInf] & \mTru[6] & \mFal[\textInf] \\\hhline{~|-----|}
      \arraySubscript{4}
        & \mFal[\textInf] & \mFal[\textInf] & \mFal[\textInf] & \mFal[\textInf] & \mFal[\textInf] \\\hhline{~|-----|}
    \end{tabular}

    \vspace{1em}
    If the edges have weights, they are stored in the matrix.\invisible{g}
\end{columns}
\vspace{2em}
\end{frame}
...
\begin{enumerate}
  ...
  \item
    ...
    \hspace{1em}If \(w\) is not visited,\\
    \hspace{2em}begin the depth-first traversal at \(w\).
\end{enumerate}
\end{frame}
...
\column{0.6\textwidth}
  ...
  \begin{center}
    ...
    \ttfamily
    \begin{tabular}{|>{\columncolor{CSUcyan}\color{black}\centering\arraybackslash}m{1.5em}|}
      \only<13>{7}\\
...
\column{0.6\textwidth}
  ...
  \begin{center}
    ...
    \ttfamily
    \begin{tabular}{r | >{\columncolor{CSUcyan}\color{black}\centering}m{0.6em} >{\columncolor{CSUcyan}\color{black}\centering}m{0.6em} >{\columncolor{CSUcyan}\color{black}\centering}m{0.6em} | l}
      \hhline{~|---|~}
      \textleftarrow{} 
      & \only<2>{0}\only<4>{1}\only<5-6>{3}\only<7-8>{4}\only<9>{2}\only<10-11>{4}\only<12>{5}\only<14>{7}\only<15>{8}%\only<17>{6}\only<19>{9} 
      &
      \only<4>{3}\only<6>{4}\only<8>{2}\only<9>{4}\only<11>{5}\only<14>{8} 
      & \only<8>{4} 
      & \textleftarrow{}
      \\
      \hhline{~|---|~}
...
\begin{columns}
  \column{0.4\textwidth}
    \centering
    \begin{tikzpicture}[very thick, -stealth, node distance=3cm, shorten >=0.5pt]
        % Vertex 0
        \alt<1>{
          \node[state] (0)              {0};
        }{
      ...
      \alt<-17>{
        ...
        \path (3) edge [out=130,in=180] node []{12} (1)
...
% \begin{tikzpicture}[very thick, -stealth, node distance=3cm]
%     \node[state] (0)              {0};
%     \node[state] (1) [above right of=0, node distance=2.12132cm] {1};
%     \node[state] (2) [right of=0] {2};
%     \node[state] (3) [below of=0] {3};
...
\begin{frame}{Shortest Path Algorithm: Enhancements}
  This demonstrated algorithm keeps track of the\\
  total weight of the shortest path. Consider what\\
  you would add to record the actual path.

  Instead of using a sequence search to find the minimum weight, which is \(O(n)\), a \emph{priority queue} could be used.
\end{frame}
