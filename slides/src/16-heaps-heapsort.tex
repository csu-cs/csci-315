...
\subtitle{CSCI 315: Data Structure Analysis\\Chapter 10 of \href{https://opendatastructures.org/ods-cpp.pdf}{Open Data Structures}}
...
\begin{frame}{Why study Heapsort?}
\begin{itemize}[<+->]
\item
...
\item
  ...
  \begin{enumerate}
  \item
    \emph{Memory}: A large area of memory from which the programmer can allocate blocks as needed, and deallocate them (or allow them to be garbage collected) when no longer needed.
  \item
...
\begin{frame}{Three Properties of a Binary Heap}
The heap data structure is a binary tree that maintains three properties, which we will discuss.\\[1em]
\begin{enumerate}
\item
  Balanced
\item
...
\item<2->
  ...
  \end{itemize}
\item<4->
  A binary tree of depth $n$ is \emph{balanced} if all the nodes at depths 0
...
\begin{frame}{Left-Justified Binary Trees}
A balanced binary tree is \emph{left-justified} if:
\begin{itemize}
\item<2->
  All levels, except possibly at depth $n$ (deepest), are filled.
\item<3->
...
\onslide<4->{\column{0.5\textwidth}\centering
  \begin{forest}
...
\onslide<5->{\column{0.5\textwidth}\centering
  \begin{forest}
    [, for tree={fit=tight, l=7mm, inner sep=0.25em}, name=root
      [
        [
...
\begin{itemize}[<+->]
\item
  A node has the \emph{heap property} if the value in the node\\
  ...
  \centerline{%
    \begin{forest}
      [, name=root, for tree={fit=tight, l=8mm, inner sep=0.25em},
        for descendants={visible on=<5->}
        [, for tree={visible on=<6->}
          [, for tree={visible on=<8->}
            [, for tree={visible on=<12->}]
            [, phantom]
          ]
...
\begin{itemize}[<+->]
\item
  Problem: Each time we add a node, we may destroy \\the heap property of its parent node.
\item
...
\begin{columns}[T]
  ...
  \item<3->
    We can sift down this node.
  \item<6->
    It is a leaf, so not children can lost the heap property.
  \end{itemize}
\column{0.6\textwidth}
\vspace{1.5cm}
  \centerline{%
...
\item<4->
  Therefore, the total time is $n$ times however long it takes the re-heap method.
\end{itemize}
\end{frame}
...
\begin{itemize}[<+->]
\item
  To re-heap the root node, we must follow \emph{one path} from the root to a leaf node (and we might stop before we reach a leaf).
\item
...
\item
  ...
  \end{itemize}
\item
  Since we re-heap inside a while loop that we do $n$ times, the total time for the while loop is $n \times O(\log n)$, or $O(n \log n$).
\end{itemize}
\end{frame}
...
\begin{itemize}[<+->]
  \item
    \emph{Efficient Operations}: Insertion and deletion are $O(\log n)$. Retrieval of the minimum (or maximum) element is $O(1)$.
  \item
    \emph{Space Efficiency}: Binary heaps can be implemented using arrays, removing the need for pointers.
  \item
    \emph{Ease of Implementation}: With arrays, the algorithms for insertion, deletion, and heapifying are intuitive.
  \end{itemize}
\end{frame}
\begin{frame}{Summary of Benefits}
  \begin{itemize}[<+->]
    \item
    \emph{Heap Sort}: An always $O(n \log{n})$ in-place sorting algorithm (i.e., $O(1)$ additional space and with predictable performance).
  \item
