...
\NewDocumentCommand{\BTreeSep}{}{%
  {\color{white}\rule[-0.5em]{1.25pt}{1.75em}}%
}
...
\begin{frame}{Example B-Tree, Order-5 with 26 Elements}
  \begin{figure}
  \centering
  \includegraphics[width=\textwidth]{order-5-b-tree-exampe}
...
\begin{itemize}
  \item<+->
    ...
    \begin{enumerate}
      ...
      \item<+->
        All non-leaf nodes except the root have at least \(\left\lceil {m / 2} \right\rceil\) children.
      \item<+->
    ...
    \centering
      \begin{forest}
        [\BTreeVal*{8}
          ...
          [
            \BTreeNode{12, 14}\BTreeSep\alt<1>{}{\BTreeVal<red!75>{17}}\BTreeNode{25, 28}%
          ]
        ]
      \end{forest}
    \end{figure}
...
Key are always inserted \emph{into leaves}.  For deletion, we wish \\to remove \emph{from a leaf}. There are four cases to consider:
