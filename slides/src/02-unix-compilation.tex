...
\begin{frame}{The Linux File Structure and Inodes}
\begin{itemize}
  ...
  \item<+-> A file has a name and an \href{https://en.wikipedia.org/wiki/Inode}{\emph{inode} (index node)} that stores its metadata or ``administrative information.''
  \begin{itemize}
    \item creation/modification date
    \item permissions
    \item properties are stored in the file's inode
...
\begin{description}[\texttt{-Wconversion~~}]
  \alt<1>{
    ...
    \item[\colorbox{codeBackground}{-Wconversion}] implicit conversions that may alter a value
...
\end{description}
\onslide<2>{}
See all options at \url{https://gcc.gnu.org/onlinedocs/gcc/Warning-Options.html}

\end{frame}

\begin{frame}{Compiling Multi-Source Programs}
\begin{itemize}
\item Compile them with:
\begin{itemize}
\item \lstinline[language={}, basicstyle=\ttfamily\color{codeVariable}]{g++ main.cpp a.cpp b.cpp -o hello}
\end{itemize}
\item Comments:
\begin{itemize}
...
\begin{tikzpicture}[thick, every node/.style={node distance=4mm, minimum height=7mm},
  ...
  % Arrows
  \draw[->, very thick] (block1) edge (block2);
  \draw[->, very thick] (block2) edge (block3);
  \draw[->, very thick] (block3) edge (block4);
  \draw[->, very thick, dashed] (block4) edge (block5);
  \draw[->, very thick] (blockLibs) edge (block4);
  \draw[->, very thick] (block5) edge (block6);

  \draw [<-, very thick] (block1) -- ++ (5cm,0) |- node[anchor=south east]{Syntax Error} (block3);
\end{tikzpicture}
\end{frame}
...
\item \lstinline{-c} tells the compiler to only create an object file.
...
\item At the heart of the operating system, the kernel, are some device drivers.
...
\column{0.5\textwidth}\vspace{1em}
  \begin{tikzpicture}[thick, every node/.style={node distance=4mm, minimum height=7mm}]
    ...
    \node[text=black, fill=CSUcyan,
      ...
      minimum width=4.1cm] (sys) {System Calls};

    \node[
      below=of sys.center,
      minimum width=4.1cm] (kernel) {Kernel};
    ...
    \node[draw, fit={(sys) (dev) (kernel)}, inner sep=0pt] (systemAll) {};
    \node[right=of systemAll] (kernelSpace) {Kernel Space};


    \node[draw,
      below=of systemAll,
      minimum width=4.1cm, align=center] (hardware) {Hardware Devices};

    % Arrows
    \draw[->, very thick] (userAll) edge node[anchor=west]{Calls} (sys);
    \draw[->, very thick] (systemAll) edge (hardware);
    \draw[->, very thick] (kernelSpace) edge (systemAll);
    \draw[->, very thick] (userSpace) edge (userAll);
...
\center{}
  ...
  \begin{tikzpicture}[thick, every node/.style={node distance=4mm, minimum height=7mm}]
    ...
    \node [draw=CSUgold, fit={(userAll) (githubText)}, fill=CSUslate, inner sep=1pt] (github) {};
    \end{scope}

    \node[text=black, fill=CSUcyan,
      minimum width=3cm] (local) [below=20mm of origin.south, anchor=north] { local };

    % Arrows
    \draw[->, very thick, color=white] (upstream) edge node[text=white, anchor=south]{fork} (origin);
    \draw[->, very thick] (origin)  to[bend right] node[anchor=east,align=right]{clone/pull} (local);
    \draw[->, very thick] (local)  to[bend right] node[anchor=west]{push} (origin);

    \onslide<2->{\draw[->, very thick] (upstream)  to[bend right] node[anchor=east]{pull} (local);}
