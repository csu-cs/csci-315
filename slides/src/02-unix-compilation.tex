...
\begin{itemize}
  \item<+-> In Linux, \href{https://en.wikipedia.org/wiki/Everything_is_a_file}{\emph{everything} is a file}! Well, almost.
  \item<+-> Drives, ports, and devices (e.g., printers) are files.
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item index node (``i'' in inode is short for ``index'')
    \item a filesystem data structure containing metadata about a file.
  \end{itemize}
  ...
  \item<+-> 
    ...
    \begin{itemize}
      ...
      \item group
      \item permissions
      \item date/time for access, modification, and status-change
      \item file size
      \item disk locations
    \end{itemize}
\end{itemize}
...
\toprule{}%
  \emph{Directory} & \emph{Description}\\
\midrule{}%
...
\item Define an optimization level by adding a number to the~\lstinline[language={}, basicstyle=\ttfamily\color{codeVariable}]{-O} flag. (\lstinline[language={}, basicstyle=\ttfamily\color{codeVariable}]{-O2} is a common choice.)
\end{itemize}

Tip: Don't optimize when debugging (\lstinline[language={}, basicstyle=\ttfamily\color{codeVariable}]{-g}) so the generated code more closely follows the source code.
\end{frame}
