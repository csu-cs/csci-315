...
\title{\Huge{}Quadtrees {\Large{}\\[-0.5em] (\& Smart Pointers)}}
  ...
  \alt<-7>{%
    ...
    \alt<-9>{%
      ...
      \alt<-20>{%
        ...
        Now the southeast child of the root has also\\
        reached the 5-point limit, so the node will also\\
        subdivide once a new point is added to it.
      }
    }
  }
}
...
\begin{frame}{Modern C++ Memory Management}
\begin{minipage}[t][3em][c]{\textwidth}
  \center{}
  In modern C++, \emph{smart pointers} are preferred for dynamic memory management, instead of raw \lstinline{new} and \lstinline{delete}.
\end{minipage}

\pause{}
\begin{itemize}[<+->]
  \item \lstinline{std::unique_ptr} is a smart pointer that owns and manages a dynamically allocated object.
  \item It automatically deletes the object when it goes out of scope, preventing memory leaks.
  \item Ownership is exclusive: cannot be copied, only moved.
\end{itemize}
\end{frame}

\begin{frame}[fragile]{\texttt{std::unique\_ptr} Example}
\begin{lstlisting}
#include <memory>
void example() {
...
\vspace{-0.5em}
\begin{itemize}
  \item The object is freed automatically when the smart pointer goes out of scope.
  \item No explicit \texttt{delete} is required.
\end{itemize}
\end{frame}
...
template<typename T>
class UniquePointer { // Incomplete Example
public:
  UniquePointer(T* datum = nullptr) : mDatum{datum}
  {}
  ...
  T* operator->() const {return mDatum;}
  T& operator*()  const {return *mDatum;}
private:
  T* mDatum;
};
...
\begin{frame}[fragile]{\texttt{std::make\_unique} vs \texttt{new}}
Preferred form:
\begin{lstlisting}
...
\vspace{-1em}
\begin{itemize}
  \item Works well with \textit{type deduction} (\lstinline{auto}) and is exception-safe.
  \item Allocation and ownership transfer happen in one step.
\end{itemize}
...
\vspace{-1em}
\begin{itemize}
  \item More verbose and unsafe if an exception is thrown between \lstinline{new} and the constructor of \lstinline{std::unique_ptr}.
\end{itemize}
\end{frame}
...
%\begin{columns}[T]
  %\column{0.5\textwidth}
    ...
    \begin{itemize}
      \item Automatic resource management.\\
        \href{https://en.cppreference.com/w/cpp/language/raii.html}{\textit{Resource Acquisition is Initialization} (RAII)}.
      \item Prevents memory leaks and dangling pointers.
      \item Exception-safe.
 ...
 % \column{0.5\textwidth}
    ...
    \vspace{-0.5em}
    \begin{itemize}
      \item Ownership semantics restrict copying.
      \item Slight performance overhead.
      \item Learning curve for ownership rules.
    \end{itemize}
%\end{columns}
\end{frame}

\begin{frame}{Other Smart Pointers}
\begin{itemize}
  \setlength\itemsep{0.5em}
  \item \lstinline{std::shared_ptr} --- shared ownership
  \item \lstinline{std::weak_ptr} --- non-owning reference to object managed by \lstinline{std::shared_ptr}
\end{itemize}
\end{frame}

\begin{frame}{Rule of 0}
\begin{itemize}[<+->]
  \item \textbf{Rule of 3}: If you define any of destructor, copy constructor, or copy assignment operator, define all three.
  \item \textbf{Rule of 0}: If you don't need to define any special member functions, don't define any.
  \item Rely on compiler-generated defaults or use smart pointers for resource management.
\end{itemize}

\onslide<4->{%
\begin{itemize}
  \item Benefits: Less code, fewer bugs, automatic resource management via RAII\@.
  \item Example: Classes with \lstinline{std::unique_ptr} members don't need custom destructors.
\end{itemize}
}
\end{frame}

\begin{frame}[fragile]{Quadtree Subtree Implementation}
  The 4 subtrees are defined using:
...
\begin{itemize}
  \item \lstinline{std::array} provides a fixed-size array with compile-time size.
  \item Each subtree is a \lstinline{std::unique_ptr} to a \lstinline{Quadtree} node.
  \item Memory is automatically managed; no manual \lstinline{delete} needed.
\end{itemize}
\end{frame}
