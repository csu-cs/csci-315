...
\begin{enumerate}
  \item \emph{Explain}: Describe the conceptual difference between stack and heap memory and how pointers reference each.
  \item \emph{Read \& Write}: Declare pointers, use \lstinline{&} and \lstinline{*}, and correctly read from and write to referenced memory.
  \item \emph{Manage Memory}: Allocate and deallocate dynamic memory with \lstinline{new}, \lstinline{delete}, and \lstinline{delete[]}, and explain common issues (memory leaks, dangling pointers).
\end{enumerate}
\end{overlayarea}
\end{frame}
...
\begin{enumerate}\addtocounter{enumi}{3}
  \item \emph{Apply Pointer Arithmetic}: Use pointer arithmetic and show equivalence between array indexing (\lstinline{ary[i]}) and pointer expressions (\lstinline{*(ary + i)}).
  \item \emph{Practice Const-Correctness}: Distinguish \lstinline{const} variants (e.g., \lstinline{const int *}, \lstinline{int * const}, \lstinline{const int * const}) and apply them appropriately.
  \item \emph{Understand Casting Risks}: Explain pointer casting (including \lstinline{reinterpret_cast}) and identify when casts cause undefined behavior.
\end{enumerate}
\end{overlayarea}
\end{frame}
...
\begin{enumerate}\addtocounter{enumi}{6}
  \item {Use Object Pointers}: Create and manipulate pointers to \lstinline{struct}/\lstinline{class} instances and use the \lstinline{->} operator.
  \item {Use Function Pointers}: Declare, assign, call, and pass function pointers (callbacks) safely.
  \item {Compare Languages}: Contrast C++ pointers with Java references and explain practical implications.
  \item {Identify Bugs}: Recognize and fix common pointer-related bugs (nullptr checks, double delete, use-after-free).
\end{enumerate}
\end{overlayarea}
\end{frame}
...
\begin{frame}{Java vs. C++ Objects}
\begin{itemize}[<+->]
\item Objects in C++ share many similarities with Java, \\
  but are considered more powerful:
\begin{itemize}
\item In C++, objects can be on the stack.
\item C++ allows multiple inheritance.
\begin{itemize}
\item no need for the \lstinline[language=Java]{interface} keyword.
\end{itemize}
...
\begin{tabularx}{1\textwidth}{>{\columncolor{codeBackground}}l >{\small}X}
  \lstinline|int *p \{\};|
    & Create a variable \lstinline{p} on the stack that will \newline{} store the memory address of an integer.\onslide<2->\\
  \lstinline{p = new int \{\};}
    & Allocate an \lstinline{int} in memory and store the memory address of that \lstinline{int} in \lstinline{p}.\onslide<3->\\
  \lstinline{*p = 5;}
    & Manipulate the new \lstinline{int} just like before.\onslide<4->\\
  \lstinline{delete p;}
    & Must deallocate because the \lstinline{int} is in the heap!\onslide<5->\\
  \lstinline{p = nullptr;}
...
we can can write \\
\lstinline{int *pX = new int[20];} \\
to allow for dynamic allocation.
\begin{itemize}
\item Usage of the array (e.g., \lstinline{x[3] = 5;} and \lstinline{pX[3] = 5;}) are identical in both cases.
\item To deallocate, use \lstinline{delete[] pX;}
\end{itemize}
\end{itemize}
\end{frame}
...
\begin{minipage}{\dimexpr\textwidth-2\fboxsep-2\fboxrule-20pt} 
...
\lstinline{std::string *pStr = new std::string("Hello!");}
...
\centering\Large
\href{https://github.com/csu-cs/CSCI-315-2026-Spring/blob/master/ethics/README.md}{Let's go over your ethics essay today!}
\end{frame}
