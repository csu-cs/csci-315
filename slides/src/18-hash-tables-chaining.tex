    ...
    \begin{tikzpicture}[
      ...
      \only<2->{
        \foreach \i [count=\k from 0] in {4,1,5,0,6,2,3} {
          ...
          \node[chain element, anchor=west, name=valval\k] at ([xshift=-0.4pt] key.east) {\(v_{\k}\)};
        }

        \foreach \i [count=\k from 0] in {7,8} {
          \node[chain element, anchor=west, name=key] at ([xshift=2.8pt] valval\k.east) {\(k_{\i}\)};
          \node[chain element, anchor=west, name=valval\k] at ([xshift=-0.4pt] key.east) {\(v_{\i}\)};
        }
      }
...
\begin{tikzpicture}[node of array/.append style = {minimum height=7mm}]
  ...
  \foreach \key/\value [count=\i] in {
    ...
    \else % All but the last row
      \node[node of array, anchor=north, name=key] at ([yshift=1pt] prev.south) {\key};%
    \fi%
    \node[node of array, right = of key.east, xshift=-0.8pt, name=valData, minimum width=1.5cm] {\value};%
