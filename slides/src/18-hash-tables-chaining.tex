...
\subtitle{CSCI 315: \textit{Data Structure Analysis}\\Chapter 5 of \href{https://opendatastructures.org/ods-cpp.pdf}{Open Data Structures}}
...
\item<4->
  ...
  \begin{itemize}
  \item
    The array concept generalized with \(O(1)\) (with some reasonable assumptions) for Insert, Delete, and Search of keys.
  \end{itemize}
\end{itemize}
\end{frame}

\begin{frame}{Array Approach --- Example}
An application searches for US citizens by\\
their social security numbers (SSN).
\begin{itemize}
\item<2->
  An array can hold references to all persons.
  \begin{itemize}
  \item
...
\begin{frame}{A Conceptual View of Hash Tables}
  %\centering
    ...
    \begin{tikzpicture}[
      ...
      \ArrayLabel{Hash Table Buckets}
      \foreach \i in {0,...,6} {
        \ArrayNodeVertical{}
      }
      
      \only<2->{%
        \foreach \i [count=\k from 0] in {4,1,5,0,6,2,3} {
          \node[chain element, anchor=west, name=key] at ([xshift=3.2pt] elAt\i.west) {\(k_{\k}\)};
          \node[chain element, anchor=west, name=valval\k, fill=CSUslate, text=white] at ([xshift=-0.4pt] key.east) {\(v_{\k}\)};
        }

        \foreach \i [count=\k from 0] in {7,8} {
          \node[chain element, anchor=west, name=key] at ([xshift=2.8pt] valval\k.east) {\(k_{\i}\)};
          \node[chain element, anchor=west, name=valval\k, fill=CSUslate, text=white] at ([xshift=-0.4pt] key.east) {\(v_{\i}\)};
        }
      }%

      % \draw[thick, fill=MyPurple,name=hashFunction]
      %   ([xshift=-3cm]elAt0.north west) rectangle ([xshift=-1.5cm]elAt6.south west);

      \onslide<3->{%
      \node[draw, thick, fill=MyPurple,
        fit=(elAt0.north west)(elAt6.south west),
        xshift=-3cm,
        minimum width=1.5cm,
        inner sep=0pt,
        outer sep=0pt, 
        name=hashFunction] {};
      \node[anchor=south, align=center, inner sep=0] at (hashFunction.north) {Hash Function (\(h\))};
      }%
      ...
      % Keys in Universe
      \node[circle, minimum size=4.2cm, draw=white, fill=CSUgold, thick, name=universe] at (-8.25,-3.75) {};
      \node[anchor=south, align=center, inner sep=0] at (universe.north) {Universe of Keys (\(U\))};

      % Seed the random number generator for reproducibility
      \pgfmathsetseed{57}

      % Loop to create 10 small circles
      \foreach \i [count=\k from 0] in {4,1,5,0,6,2,3,4,1} {
        % Generate random angle and radius for polar coordinates within the large circle
        \pgfmathsetmacro{\angle}{random(0,360)}
        \pgfmathsetmacro{\radius}{sqrt(random()) * 1.8} % scale radius to stay within the larger circle

        % Convert polar to Cartesian coordinates
        \pgfmathsetmacro{\x}{\radius * cos(\angle) - 8.25}
        \pgfmathsetmacro{\y}{\radius * sin(\angle) -3.5}
        \pgfmathsetmacro{\elNum}{int(mod(\i, 7))}
        ...
        % Draw line
        \only<4->{
          \draw[-stealth, very thick] (\x, \y)[rounded corners] -- (hashFunction.center |- \x,\y) -- (index\elNum);
        }

        % Label the circle
        \begin{pgfonlayer}{foreground layer}
          \node[text=black, anchor=north east, inner sep=0, outer sep=0] at (\x,\y) {\(k_{\k}\)}; 
        \end{pgfonlayer}
...
%       \node[chain element, anchor=west, name=valval] at ([xshift=-0.8pt] key.east) {val5};
...
%       \node[chain element, anchor=west, name=valval] at ([xshift=-0.8pt] key.east) {val4};
%       \begin{scope}[on background layer]
%         \node[chain bucket,fit=(key) (valval)] (bucket) {};
%       \end{scope}
%       \draw[link] (ptr) -- (bucket);
...
%       \node[chain element, anchor=west, name=valval] at ([xshift=-0.8pt] key.east) {val3};

%       \node[chain element, anchor=west, name=key2] at ([xshift=5pt] valval.east) {key = 28};
%       \node[chain element, anchor=west, name=valval2] at ([xshift=-0.8pt] key2.east) {val2};
...
%       \node[chain element, anchor=west, name=valval] at ([xshift=-0.8pt] key.east) {val1};
%       \begin{scope}[on background layer]
%         \node[chain bucket,fit=(key) (valval)] (bucket) {};
%       \end{scope}
%       \draw[link] (ptr) -- (bucket);
...
\begin{frame}[fragile]{Hash Function}
  ...
  \[
    \begin{tikzcd}[
      %every label/.append style={execute at begin node=\mathsf}
    ]
        \mathsf{key} \arrow[d, "hash"] \\
        \mathsf{hash~code} \arrow[d, "compress"] \\
        \mathsf{bucket~index}
    \end{tikzcd}
  \]
\end{frame}
...
\begin{frame}{Hash Collision Example}
Example: Insert the keys \(2, 21, 34, 54, \ldots{} \) into an array\\
with capacity \(5\) using \(h (k) = k \bmod 5\), where \(k\) is the key.%
\vspace{-0.5em}
\center%
\begin{columns}[]%
  \column{0.3\textwidth}
  \center%
\begin{tikzpicture}[subcomponent of array el/.append style = {node distance=-0.4pt and -0.8pt, minimum width=1.5cm, fill=CSUslate, text=white}]
  
  \ArrayLabel{}
  \ArrayNodeVertical{,}%
  \node[, above = 0pt of val, text depth=0.2ex] {key};%
  \node[, above = 0pt of lastElValue, text depth=0.2ex] {value};%

  \node[, above = 1.5em of prev.north, anchor=south]{%
    \alt<-2>{Insert \(2\)}{%
      \alt<-4>{Insert \(21\)}{%
        \alt<-6>{Insert \(34\)}{%
          Insert \(54\)%
  ...
  \ArrayNodeVertical{\only<4->{21},\only<4->{\ldots{}}}
  \ArrayNodeVertical{\only<2->{2},\only<2->{\ldots{}}}
  \ArrayNodeVertical{~,~}
  \alt<-7>{  
    \ArrayNodeVertical{\only<6-7>{34},\only<6-7>{\ldots{}}}
  }{
    \begin{scope}[node of array/.append style = {fill=red},
        subcomponent of array el/.append style = {fill=CSUslate!50!red, text=white}
    ]
      \ArrayNodeVertical{54,\ldots{}}
    \end{scope}
  }
...
%       \node[node of array, anchor=north, name=key] at ([yshift=1pt] prev.south) {\key};%
%     \fi%
%     \node[node of array, right = of key.east, xshift=-0.8pt, name=valData, minimum width=1.5cm, fill=CSUslate, text=white] {\value};%
%     \ifnum\i=1 % First element
%       \node[, above = 2pt of valData] {value};%
%     \fi%
%     \node[array index, anchor=east] at ([xshift=-0.8pt] key.west) {[\thearrayIndex]};%
...
\begin{frame}{Choosing a Hash Function}
  \begin{figure}
    %\centering
    \caption{Distribution of the people's surname initial letter according\break to the 2010 US Census.}
...
\begin{itemize}
  ...
  \item<3->
    ...
    \item
      ...
      \(p\) is a arbitrary \emph{prime}, greater than \(m\) (often \(2^{31} - 1\)) \\
...
\onslide<2->{Two ways to resolve collision are:}
\begin{itemize}
  \item<3->
