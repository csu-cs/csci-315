...
\begin{frame}{Introduction}
  \begin{itemize}[<+->]
  \item
    The ``highest-priority'' element is the first one removed.\\
...
\begin{itemize}
\item<2->
  \lstinline{PriorityQueue()}: a constructor
\item<3->
  \lstinline{void add(const Type &val)}: inserts \texttt{val} into the queue
\item<4->
  \lstinline{Type& pop()}: removes and returns the highest-priority element
\item<5->
  \lstinline{const Type& top() const}: returns (but does not remove) the highest-priority element
\item<6->
  \lstinline{bool isEmpty() const}: returns true IFF empty
\item<7->
  \lstinline{unsigned int size() const}: returns the element count
\item<8->
  \lstinline{void clear()}: discards all elements
...
\item
  ...
  \item
    If we load a huge data and expect to do only a few searches, we probably \emph{don't} want to spend time sorting.
  \end{itemize}
\item
...
\begin{itemize}[<+->]
\item
  A \emph{priority queue} is a data structure that is designed to return elements in order of priority.
\item
