...
Implemented as a vector of lists. Each list node represents\\ an edge, storing the destination vertex and edge weight.\vspace{-2em}
\begin{columns}
\column{0.33\textwidth}
  \pause%
    \begin{figure}
  ...
  \begin{figure}
    ...
    \begin{tikzpicture}[node of array/.append style={fill=CSUgold}, node of array/.append style = {minimum width=6mm, minimum height=11mm}, node of list/.append style={minimum width=6mm, minimum height=6.5mm, inner sep=3pt}, additional node of list/.append style={minimum width=7mm}, link pointer of list/.append style={minimum height=6.5mm, minimum width=4.5mm}]
      ...
      \ArrayNodeVertical{}
        ...
        % Label array index
        \onslide<4->{%
          \node[anchor=south west, font={\small}, text depth=.2ex, name=source, inner sep=1pt, text=CSUcyan!50!white] at ([yshift=9pt, xshift=0pt] index0.north west) {source};
          \draw[link, ->, draw=CSUcyan!50!white] (source) to[bend right=5] (index0);
        }
        ...
        % Label a node components destination and weight
        \onslide<5->{
          \node[anchor=south east, font={\small}, text depth=.2ex, name=dest, inner sep=1pt, text=CSUcyan!50!white] at ([yshift=10pt, xshift=-1pt] firstElValue.north east) {destination};
          \draw[link, ->, draw=CSUcyan!50!white] (dest) to[bend left=5] (firstElValue);
          \node[anchor=south west, font={\small}, text depth=.2ex, name=weight, inner sep=1pt, text=CSUcyan!50!white] at ([yshift=10pt, xshift=1pt] lastElValue.north west) {weight};
          \draw[link, ->, draw=CSUcyan!50!white] (weight) to[bend right=5] (lastElValue);
        }
      \ArrayNodeVertical{}
        \coordinate (ptr) at (val);
        \fill (val) circle(2pt);
...
\column{0.67\textwidth}
  \begin{figure}
    \centering
    \begin{tikzpicture}[node of array/.append style={fill=CSUgold, minimum width=6mm, minimum height=8.5mm}, node of list/.append style={minimum width=6mm, minimum height=5.75mm, inner sep=3pt}, additional node of list/.append style={minimum width=4.5mm}, link pointer of list/.append style={minimum height=5.75mm, minimum width=4.5mm},   span of array/.append style={minimum height=0mm}]
      \ArrayLabel{}
...
\begin{frame}{Minimal Spanning Tree Algorithms}
  \large\center%
To find the edge with the minimum weight, consider using a heap (e.g., \href{https://en.cppreference.com/w/cpp/container/priority_queue.html}{\texttt{std::priority\_queue}}).
\end{frame}
...
\begin{itemize}
  \item
    \texttt{vertexCount}: the vertex count. A spanning tree has exactly \lstinline{vertexCount - 1} edges.
  \item
...
% \While{\!edgeQueue.empty() \&\& edgeCount \< vertexCount}
...
Function: prims(startIdx):
  ...
  return (edgeCount != vertexCount) ? -1 : totalCost
\end{lstlisting}
\end{frame}
...
\begin{frame}{Performance}
  \large
  What operations are more efficient with an\\
  \emph{Adjacency Matrix}?

  \pause{}
  \begin{itemize}
    \item
      Faster for checking if an edge exists between two specific vertices (\(O(1)\) lookup).
    \item
...
\begin{frame}{Performance}
  ...
  \begin{itemize}
  \item
    Faster for iterating over edges (\(O(V+E)\)),
  \item
    Supports parallel edges.
  \item
    Allows Prim's algorithm to run in \(O(E \log{E} + V \log{E}) = O((V + E) \log{E})\) time.
  \end{itemize}
\end{frame}
...
\begin{frame}{Performance}
  \large
    ...
    \begin{itemize}
      \item
        \emph{Adjacency lists} are far more efficient for \textit{sparse} graphs (few edges relative to vertices) with \(O(V+E)\) instead of \(O(V^2)\) memory.
      \item
        \emph{Adjacency matrices} always use \(O(V^2)\) space regardless of edge count, but no pointers are required.
    \end{itemize}
\end{frame}
