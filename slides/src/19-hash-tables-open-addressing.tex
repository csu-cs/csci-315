...
\begin{frame}{Dealing with Collisions}
  ...
  the same array entry --- this is called a \emph{collision}.
...
\begin{frame}{Hash Tables with Open Addressing}
  ...
  \begin{tikzpicture}[%
    ...
    subcomponent of array el/.append style={minimum width=16mm, node distance=-1.4pt},%
  ]%
    \ArrayLabel{}
    ...
    \alt<-5>{
      ...
      \ArrayNodeVertical[]{15,$v_{3}$}
    }
    \onslide<6->{\node[name=bucketIndex, anchor=west, text depth=.1ex] at ([xshift=3pt]lastElValue.east) {index is \texttt{7}};}

    % Add instructions (what to insert) at the top.
    \node[anchor=south, inner sep=0] at (current bounding box.north) {
...
\begin{frame}
  \center%
  \Large%
  Let's implement using open-addressing\\ and simple linear probing.
\end{frame}
...
%\end{figure}
\column{0.4\textwidth}
Hash tables that far exceed the CPU\\ cache size, linear probing performs better due to better locality of reference. However, linear probing performs poorly as the table gets full.
\end{columns}
\end{frame}
...
\begin{tikzpicture}[%
  ...
  \ArrayNodeVertical{1, $v_2$}
  \alt<-3>{
    \ArrayNodeVertical{2, $v_1$}
  }{
    \alt<-7>{
      \begin{scope}[node of array/.append style = {fill=CSUslate, text=white!50!CSUslate}]
        \ArrayNodeVertical{2, $v_3$}
      \end{scope}
    }{
  ...
  \onslide<6-7>{%
    ...
    \node (where) [right=of lastElValue.north east] {Where is 7?};
    \draw[link, -stealth] (where) to[bend left=20] (prev);%
  }
  \alt<1>{
    \begin{scope}[node of array/.append style = {fill=CSUslate, text=white!50!CSUslate}]
      \ArrayNodeVertical{?,?}
    \end{scope}
  }{\ArrayNodeVertical{7, $v_4$}}
