...
\begin{itemize}
    \item Identify and use common Unix/Linux commands \\for \emph{file and directory management}.
    \item Operate basic \emph{text editors} in a Unix environment.
    \item Understand and apply \emph{file permissions and user/group management} commands.
    \item Manipulate command-line data with \emph{input/output redirection and pipes}.
    \item Utilize \emph{regular expressions} for pattern matching in file operations.
    \item Locate and interpret \emph{system and user information} using Unix commands.
\end{itemize}
\end{frame}
...
\begin{tabular}{c l}%
  \texttt{:q}  & to quit from vim\\
  \texttt{:wq} & to write changes and quit\\
...
\column{0.40\textwidth}
  \only<1>{\includegraphics[width=1\textwidth]{vi-meme}}
  \only<2>{\includegraphics[width=1\textwidth]{vim}}
\end{columns}
...
\column{0.4\textwidth}
  ...
  \end{itemize}
\column{0.6\textwidth}
  \only<1>{\includegraphics[width=1\textwidth]{nano}}
  \only<2>{\centerline{\includegraphics[width=0.9\textwidth]{nano-meme}}}
\end{columns}
\end{frame}
...
\begin{frame}{\texttt{\textbf{touch}} -- Create empty FILE(s).}
...
Usage: \lstinline{touch [OPTIONS] [FILES]}
\begin{itemize}
  \item \lstinline{touch data1.txt}
  \item \lstinline{touch data2.txt data3.txt temp/data4.txt}
  \item Options:\\
  \begin{tabular}{l l}
    \lstinline{-c} & Do not create any files.\\
    \lstinline{-a} & Change only the access time.\\
    \lstinline{-m} & Change only the modification time.\\
  \end{tabular}
\end{itemize}
\end{frame}
...
\item
  ...
  \lstinline{su - root}
\item
  In many Linux distributions, the root user is disabled by default. Instead, \lstinline{sudo} can be added to the beginning of a command to temporarily grant the command root permissions. This gives admins greater control of what has the power to do anything on the system.
\end{itemize}
\end{frame}
