...
\begin{itemize}
    ...
    \item Choose and operate basic text editors in a Unix environment.
...
% \subsection{}
\begin{frame} {ls -- List files and directories.}
...
\begin{frame}{cd -- Change directories (change the working directory).}
...
\begin{frame}{Making Directories}
...
\begin{frame}{Removing Directories}
\lstinline{rmdir DIRECTORY} -- Remove the DIRECTORY(ies) if they are empty.
  \begin{tabularx}{1\textwidth}{l X}
...
\begin{itemize}
\item Any command that accepts a file or folder as a parameter assumes that the file is in the present working directory (PWD) unless told otherwise.
