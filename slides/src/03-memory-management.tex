...
\subtitle{CSCI 315: \textit{Data Structure Analysis}\\Chapter 12}
...
\begin{enumerate}
\item 
  A \textit{reference} parameter holds the memory \\
...
\begin{frame}{A Review of C++ References \& Aliases}
  \begin{center}\Large
  Let's test out some example code for aliases and references.
\end{center}
\end{frame}

\section{Static Data}

\begin{frame}{Three Memory Regions (for Data Storage)}
\begin{columns}
\column{0.67\textwidth}
\begin{overlayarea}{\linewidth}{12\baselineskip}
  \begin{itemize}[<+->]
  \item
    \textbf{Static storage}: allocated before program execution and remains for the program's lifetime.
  \item
    ...
    \end{itemize}
  \item
    \textbf{Heap}: dynamically allocated storage, managed by the runtime allocator.
  \end{itemize}
\end{overlayarea}
\column{0.33\textwidth}
  \vspace{1.5em}
  \input{../imgs/memory-management/memory-structure}
\end{columns}
...
\begin{columns}
  \column{0.67\textwidth}
  \begin{overlayarea}{\linewidth}{10.5\baselineskip}
  \begin{itemize}[<+->]
  \item
...
\column{0.67\textwidth}
  ...
  size and layout are \emph{known at compile time}.
  \item
...
\begin{frame}{Stack Resuse}
  \begin{center}\Large
  Let's look at an example side effect of not initializing variables on the stack.
\end{center}
\end{frame}
...
\column{0.67\textwidth}
  ...
  \begin{itemize}
  \item
    The call stack and heap grow towards each other (in many implementations) as required by program events.
  \item
...
\begin{frame}{Heap Management Operations}
\begin{columns}
\column{0.67\textwidth}
  \begin{itemize}[<+->]
  \item
...
\begin{frame}{Heap Allocation Failure}
\begin{columns}
\column{0.67\textwidth}
  \begin{itemize}[<+->]
  \item
    An allocation can fail when the program cannot obtain enough storage. By
  default, \lstinline{new} throws \lstinline{std::bad_alloc}; exception.
  \item
...
\column{0.67\textwidth}
  \begin{itemize}[<+->]
  \item
    \lstinline{delete} releases a storage block to the allocator.
  \item
    The storage block's status is set to \textit{unused} and
...
\begin{frame}{Heap Fragmentation Example}
  \begin{center}\Large
  Let's look at an example of how the heap can become fragmented.
\end{center}
\end{frame}
...
A pointer must point to a valid object before you can \textit{dereference}\\ it (follow
the pointer).\\[1em]
...
\end{lstlisting}
  \pause%
  \centering\textbf{Undefined Behavior! \texttt{p} holds an indeterminate location.}
\column{0.5\textwidth}
  \pause%
\begin{lstlisting}
int foo;
...
\end{lstlisting}
  \pause%
  \centering\textbf{This is fine.\ \lstinline{p} points to \lstinline{foo}.}
\end{columns}
\end{frame}
...
\lstinline|int *pNum = &num;| & \raisebox{-.5\dimexpr\totalheight-\ht\strutbox}{\onslide<2->{%
\begin{tikzpicture}[pointer/.append style = {thick}, node of list/.append style = {thick, node distance=10mm}]
  ...
  \node[inner sep=0pt, align=center, anchor=south, text depth=.25ex] at ([yshift=1mm] firstElValue.north) {some \texttt{int}};
...
\lstinline|double *pDub;| & \raisebox{-.5\dimexpr\totalheight-\ht\strutbox}{\onslide<3->{%
\begin{tikzpicture}[pointer/.append style = {thick}, node of list/.append style = {thick, node distance=10mm}]
  \LinkPtr{\texttt{pDub}}
  \node[node of list, right = of ptr, name=firstElValue, minimum width=30mm,] {?};
  \node[inner sep=0pt, align=center, anchor=south, text depth=.25ex] at ([yshift=1mm] firstElValue.north) {some \texttt{double}};
  \draw[link] (ptr) to[bend left=8] (firstElValue);
\end{tikzpicture}}%
}\\[0.5em]
...
\lstinline|int **ppNum = &pNum;| & \raisebox{-.5\dimexpr\totalheight-\ht\strutbox}{\onslide<4->{%
\begin{tikzpicture}[pointer/.append style = {thick}, node of list/.append style = {thick}]
  \LinkPtr{\texttt{ppNum}}
  
  \node[pointer, name=ptr2, node distance=6.915mm, right = of ptr2] {};
  \node[node of list, node distance=6.915mm, right = of ptr2, name=firstElValue, minimum width=20mm,] {?};
  
  \fill (ptr2) circle(2pt);
  
  \node[inner sep=0pt, align=center, anchor=south, text depth=.25ex] at ([yshift=1.5mm] ptr2.north) {\texttt{pNum}};
...
\begin{center}
\lstinline{int *pArr = new int[10];}\\\vspace{1em}

\input{../imgs/memory-management/memory-allocation-heap-array.tex}
\end{center}
\end{frame}
...
\item
  ...
  \begin{itemize}
  \item
    Creates a memory block of 5 integers on the stack (\(5 \times 4\) bytes).
  \item
...
\begin{itemize}
\item
  Integer arithmetic (\texttt{+}, \texttt{-}, \texttt{++}, \texttt{--}, \texttt{+=}, \texttt{-=}) works with array pointers.
\item
...
\begin{frame}{Pointer Arithmetic}
  \begin{center}\Large
  Let's look at an example using\\ pointer arithmetic.
\end{center}
\end{frame}
...
Each of the above statements has \emph{undefined behavior} and may result in a \emph{segmentation fault}.
...
without first being deallocated.\vspace{1em}

Example:
\begin{lstlisting}
int *p = new int; // An int value's address
p = nullptr; // Allocated int is now unreachable.
\end{lstlisting}

\begin{itemize}[<+->]
\item
  Result?
\item\textbf{Memory Leak! (Orphan Blocks)}\\
...
\item
  and all or part of the system or device stops working correctly,
...
\begin{frame}{Memory Leaks}
  ...
  \href{https://valgrind.org/downloads/guis.html}{Graphical User Interfaces}.
  
  * The \lstinline{-g} includes debug information, making Valgrind reports easier
  to interpret.
\end{block}
\end{frame}
...
\begin{itemize}
\item
  A second problem can occur when multiple \\
  pointers are assigned to a block of heap memory.
\item
  Deleting the block does not change any pointer automatically. All
  pointers to that block must be set to \lstinline{nullptr} or they are \emph{dangling}.
\item
...
\begin{frame}{Memory Leaks and Garbage Collection}
  The popularity of \textit{object-oriented programming} has \\ led to an
  emphasis on heap storage management.\\[1em]
...
\begin{itemize}
\item
  A garbage collector identifies unreachable objects and reclaims their storage.
\item
  Some garbage collectors also \emph{compact} the heap by moving objects together to reduce fragmentation.
\end{itemize}
\end{frame}
...
\item
  ...
  storage block isn't explicitly freed, it becomes \\garbage.
  \begin{itemize}
  \item
    Non-standard garbage collectors are available for C++.
  \end{itemize}
\item
  Java, C\#, Python, JavaScript, Perl, and most scripting languages perform garbage collection.
\item
...
\begin{frame}
\centering\Large
Why do you think C/C++ include\\ garbage collection?
\end{frame}
...
\begin{itemize}
\item
  \emph{Garbage}: dynamically allocated storage that is no longer reachable by
...
\item
  ...
  \item
    A node is unlinked from a linked data structure, but isn't deallocated.
  \item
    \textellipsis{}
  \end{itemize}
\end{itemize}
...
\begin{itemize}
\item
  A \textbf{dangling pointer} (or dangling reference) is a pointer (reference)
  that refers to an object whose lifetime has ended.
\item
  An \textbf{orphan (or garbage)} is a dynamically allocated memory block that is no
...
\begin{center}\Large
Check your understanding with the \href{https://csu-cs.github.io/csci-315/lectures/03-memory-management\#review-questions}{posted review questions} (not for a grade).

\vspace{1em}
Then, complete Lab 3 (on Blackboard).
\end{center}
\end{frame}
