...
\begin{itemize}
  ...
  \item
    \emph{Asymptotic}: the study of the function $f$ as $n$ becomes larger and larger without bound.
  \item
...
\small
\emph{Comparison-based search algorithms}: Search a list by comparing the target element with list elements.
\pause{}
...
\begin{tikzpicture}[node of array/.append style={minimum width=18mm, inner sep=1pt}]
  ...
  \onslide<6>{\draw[link, <-] ([xshift=-2mm] el25.south east) to[bend right] node[below] {move} ([xshift=2mm] el23.south west);}
  ...
  \onslide<18>{\draw[link, ->] (el17) to[bend left=50] node[below] {move} (el18);}
