...
\item
  ...
  \end{itemize}
\item
  The comparison tree is a type of \emph{Binary Tree} because each comparison
...
[{\(L_0:L_1\)}, name=root,
    [{\(L_1:L_2\)}, edge label = {node [midway,below,sloped] {\(L_0 \le L_1\)} }
      [{\(\mathbf{0, 1, 2}\)}, edge label = {node [midway,below,sloped] {\(L_1 \le L_2\)} }]
      [{\(L_0:L_2\)}, edge label = {node [midway,above,sloped] {\(L_1 > L_2\)} }
        [{\(\mathbf{0, 2, 1}\)}, edge label = {node [midway,below,sloped] {\(L_0 \le L_2\)} }]
        [{\(\mathbf{2, 0, 1}\)}, edge label = {node [midway,above,sloped] {\(L_0 > L_2\)} }]
      ]
    ]
    [{\(L_1:L_2\)}, edge label = {node [midway,above,sloped] {\(L_0 > L_1\)} }
      [{\(L_0:L_2\)}, edge label = {node [midway,below,sloped] {\(L_1 \le L_2\)} }
        [{\(\mathbf{1, 0, 2}\)}, edge label = {node [midway,below,sloped] {\(L_0 \le L_2\)} }]
        [{\(\mathbf{1, 2, 0}\)}, edge label = {node [midway,above,sloped] {\(L_0 > L_2\)} }]
      ]
      [{\(\mathbf{2, 1, 0}\)}, edge label = {node [midway,above,sloped] {\(L_1 > L_2\)} }]
    ]
]
\end{forest}
\end{frame}
...
\item<+->
  ...
  associated with each root-to-leaf path, because\\ the sort algorithm only compares and moves the data.
\item<+->
...
\begin{frame}{Lower Bound on Comparison-Based Sorting}
\large
Theorem: Any algorithm that sorts a \(n\) \\
elements (by comparison of only the keys) \\
makes \emph{at least} \(O(n\log_2n)\) key comparisons\\
in its worst case.
...
\emph{Quick Sort}: uses the \textit{divide-and-conquer} technique.
...
\begin{itemize}\setlength\itemsep{0.5em}
\item
  Move elements less than the pivot value to the\\ \textit{lower} sub-list.
\item
  Move elements greater than the pivot value to the \textit{upper} sub-list.
\item
...
\begin{itemize}\setlength\itemsep{0.5em}
  ...
  \begin{itemize}
    \item
      \textit{Insertion Sort} is best for lists of 10 or fewer elements.
    \item
  ...
  \begin{itemize}
    \item
      \textit{Hoare's partitioning}: Traverse the list from both sides, swapping smaller elements to the left and larger elements to the right.
    \item
      \textit{Dual-Pivot partitioning}: Partitions the list into three sub-lists.
  \end{itemize}
\end{itemize}
\end{frame}
