...
\subtitle{CSCI 315: \textit{Data Structure Analysis}\\Chapter 16}
...
\midrule
  ...
  Remove from End       & \onslide<5->{$O(n)$ else wasted space} \\
...
\item
  The first node is called the \emph{head}; the last is the \emph{tail}.
\end{itemize}
\center%
\begin{tikzpicture}
...
\begin{itemize}
\item
  Has a \emph{variable size};\\
  memory is allocated and released as needed.
\item
  Some algorithms are \emph{faster} with linked lists compared to arrays.
\item
  Expandable to \emph{nonlinear data structures}\\
  (i.e., nodes with multiple pointers)\\
...
\begin{frame}{Traversing a Linked List}
...
remember to also update \lstinline{mpTail} and \lstinline{mLength}.
\end{frame}
...
std::string LinkedList::removeAt(int index) {
  ...
  else if (index >= mLength) { // true for empty lists
    value = removeLast();
  }
  else {
