...
\begin{itemize}
\item
  Additional memory. Each pointer requires additional 4 or 8 bytes, be
  which may significant with large data sets.
\item
  Discontinuous memory.
\item
  Some algorithms are slower on Linked Lists, compared to arrays.
\end{itemize}
\end{frame}
...
\begin{itemize}
\item<1->
  %\raisebox{-.5\height}{\parbox{0.5\textwidth}{%
    Variable declaration:\\
    \lstinline|Node *pHead{nullptr};|%}
    % \raisebox{-.5\height}{\onslide<2->\begin{tikzpicture}
    %   \LinkPtr{pHead}
    % \end{tikzpicture}}%
  %}
\item<3->
  %\raisebox{-.5\height}{\parbox{0.5\textwidth}{%
    ...
    % \raisebox{-.5\height}{\onslide<4->\begin{tikzpicture}
    %   \LinkedList{\textquotesingle{}A\textquotesingle{}}
    % \end{tikzpicture}}%
  %}
\item<5->%
  %\raisebox{-.5\height}{\parbox{0.55\textwidth}{%
    Add another value:\\
    \lstinline|pHead->pNext = new Node{'B'};|%}
    % \raisebox{-.5\height}{\onslide<6->\begin{tikzpicture}
    %   \LinkedList{\textquotesingle{}A\textquotesingle{}, \textquotesingle{}B\textquotesingle{}}
    % \end{tikzpicture}}%
  %}
\item<7->
  Add a third value:\\
...
%     \begin{tikzpicture}
%       \LinkedList{\textquotesingle{}A\textquotesingle{}, \textquotesingle{}B\textquotesingle{}, \textquotesingle{}C\textquotesingle{}}
%     \end{tikzpicture}}
\end{frame}
...
% \begin{frame}{}
% \centering
% {\Large Prepending a Value to a List}
% \\[3em]
% \begin{tikzpicture}
%   \LinkedList[tail]{\textquotesingle{}A\textquotesingle{}, \textquotesingle{}B\textquotesingle{}, \textquotesingle{}C\textquotesingle{}, \textellipsis, \textquotesingle{}Z\textquotesingle{}}
% \end{tikzpicture}
% \end{frame}
...
% \begin{frame}{}
% \centering
% {\Large Appending a Value to the End of a List}
% \\[3em]
% \begin{tikzpicture}
...
void LinkList::insert(const std::string &val, int index) {
  if (index <= 0) {
    ...
    // Traverse to location and insert.
  }
}
\end{lstlisting}
\end{frame}
...
\begin{itemize}
\item Before deleting, save the old head and update head.\\
\begin{lstlisting}[basicstyle=\small\ttfamily]
...
\item Update current's next to null.\\
  \lstinline{pCurr->pNext = nullptr;}\\[1em]
\item<3-> Delete tail and point to new tail.\\
  \lstinline{delete mpTail;}\\
  \lstinline{mpTail = pCurr;}\\[1em]
\end{enumerate}
...
\item Create a pointer to the node to remove.\\
  \lstinline{Node *pDel = pPrev->pNext;}
\item<3->
  Set \texttt{pPrev}'s next point to the node after the deletion.\\
  \lstinline{pPrev->pNext = pDel->pNext;}
\item<5->
  Delete the node.\\
...
What is the Big $O$ of the following operations on linked lists?
