...
\begin{frame}{Array-Based Queue Implementation}
  ...
  \begin{enumerate}
    ...
    \item
      \lstinline{mFrontIndex} to track the first element.
    \item
      \lstinline{mRearIndex} to track the last element
    \item
      \lstinline{mCapacity} to specify the maximum size of the queue.
  \end{enumerate}
\end{frame}
...
\begin{tikzpicture}[stack instruction/.append style={fit={(queueMembers.east) (queueMembers.west)}, yshift=5em}]
  ...
  \ArrayNode{}
    ...
    \begin{scope}[on background layer]
      \node[draw,fill=CSUslate, very thick,fit=(mpList) (mFrontIndex) (mRearIndex) (mCapacity)] (queueMembers) {};
    \end{scope}
...
\begin{frame}{Circular Array: Full or Empty?}
  \begin{itemize}
    \item Problem: In the previous two examples, notice that the state of \lstinline{mFrontIndex} and \lstinline{mRearIndex} is the same for full and empty queues.
...
\begin{frame}{Linked-List Based Queue Implementation}
  ...
  \item
    ...
    \item
      Requires array to be treated in a specific way, together with \lstinline{mFrontIndex} and \lstinline{mRearIndex}.
    \end{itemize}
  \item
...
\begin{frame}{Enqueue (Push)}
  ...
  \begin{tikzpicture}
    ...
    \alt<-3>{
      ...
      \TailPtr[null]{mpTail}
      \draw[link] (tPtr) to[bend right=40] (newTail);
    }
